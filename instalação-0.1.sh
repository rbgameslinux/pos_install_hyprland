...
if [[ "$resposta" == "sim" || "$resposta" == "s" ]]; then
 ...
 # pergunta se a gpu é AMD ou Nvidia

 read -p "Sua GPU é AMD? (sim/não): " resposta
...
echo "Você escolheu NÃO. Executando a instalação p/Nvidia"
echo
